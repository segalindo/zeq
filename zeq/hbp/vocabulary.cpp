...
#include <zeq/hbp/binarySetOperation_generated.h>
#include <zeq/event.h>
...
const std::pair< uints, uints>& binarySet )
{
  zeq::Event event(EVENT_BINARYSETOPERATION);

//  uints serialized;

  const uints& first = binarySet.first,
       & second = binarySet.second;
//  serialized.reserve( first.size( ) + second.size( ) + 1);
//
//  serialized[0] = first.size( );
//
//  serialized.insert(serialized.end( ), first.begin( ), first.end( ));
//  serialized.insert(serialized.end( ), second.begin( ), second.end( ));

  flatbuffers::FlatBufferBuilder& fbb = event.getFBB();

  auto firstData = fbb.CreateVector( first );
  auto secondData = fbb.CreateVector( second );


  BinarySetOperationBuilder builder( fbb );
